...
public class UpdateNodesFromLink extends VoltProcedure {
	 public final SQLStmt getLinks = new SQLStmt(
		        "SELECT id2 FROM " + LinkbenchConstants.TABLENAME_LINK +
		        " WHERE id1 = ?"
		    );
	 public final SQLStmt updateNodes = new SQLStmt(
			 "UPDATE " + LinkbenchConstants.TABLENAME_NODE +
			 " SET version = ? , time= ? , data= ?  WHERE id = ?"
			 );
	 public VoltTable[] run(int nid) {
	     ...
	     if(Math.min(result[0].getRowCount(),LinkbenchConstants.LIMIT_MULTITOCH) > 0){
	            ...
	            for (Integer index : indices) {
	            	String data = "abcdefg";
	                voltQueueSQL(updateNodes, 1, 1, data,linking[index]);
	            }
